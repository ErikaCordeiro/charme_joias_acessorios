...
import { defaultHomeContent, mapHomeContentToForm } from '../helpers/homeContent'
...
function Home() {
  ...
  useEffect(() => {
    ...
    const loadHomeContent = async () => {
      try {
        const response = await api.get('/site/home-content')
        if (mounted) {
          setHomeContent(mapHomeContentToForm(response.data))
        }
      } catch (loadError) {
        console.error('Erro ao carregar conteudo da home:', loadError)
      }
    }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fbf8f1]">
      <header className="sticky top-0 z-50 border-b border-[#062f35]/10 bg-[#fbfaf7]/98 backdrop-blur">
        <div className="mx-auto flex min-h-[56px] w-full items-center gap-2 px-2 sm:min-h-[62px] sm:gap-4 sm:px-4 lg:px-8">
          <nav className="hide-scrollbar flex min-w-0 flex-1 items-center gap-4 overflow-x-auto whitespace-nowrap pr-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[#062f35] sm:gap-5 sm:text-[11px] lg:justify-center lg:gap-9 xl:gap-11">
            {menuItems.map((item) => (
              <Link
                key={item.label}
                to={item.to}
          ...
          <div className="flex shrink-0 items-center gap-2 border-l border-[#062f35]/10 bg-[#fbfaf7] pl-2 text-[#062f35] sm:gap-3 sm:pl-4 lg:gap-5">
            <button type="button" aria-label="Buscar" onClick={() => setIsSearchOpen(true)} className="transition hover:text-[#0A6772]">
              <svg aria-hidden="true" viewBox="0 0 24 24" className="h-[18px] w-[18px] fill-none stroke-current stroke-[1.6] sm:h-6 sm:w-6">
                <circle cx="11" cy="11" r="7" />
                <path d="m16.5 16.5 4 4" />
              </svg>
            </button>
            <button type="button" aria-label="Favoritos" onClick={() => navigate('/products')} className="transition hover:text-[#0A6772]">
              <svg aria-hidden="true" viewBox="0 0 24 24" className="h-[18px] w-[18px] fill-none stroke-current stroke-[1.5] sm:h-6 sm:w-6">
            ...
            <button type="button" aria-label="Sacola" onClick={() => navigate('/cart')} className="relative transition hover:text-[#0A6772]">
              <svg aria-hidden="true" viewBox="0 0 24 24" className="h-[18px] w-[18px] fill-none stroke-current stroke-[1.35] sm:h-6 sm:w-6">
                <path d="M6.5 8.5h11l.8 11H5.7l.8-11Z" />
                <path d="M9 8.5a3 3 0 0 1 6 0" />
              </svg>
              <span className="absolute -right-1.5 -top-2 flex h-4 w-4 items-center justify-center rounded-full bg-[#062f35] text-[0.58rem] font-bold text-white">0</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <HomeBrandSection content={homeContent} />
